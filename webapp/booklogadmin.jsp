...
<body>
    ...
    <div class="container">
        ...
        <div class="add-button">
          <a href="logadd.jsp"><button type="button">Add Book</button></a>
        </div>
        
        <form action="logcrud.jsp" method="POST">
            <input type="text" name="rollno" placeholder="Enter Roll Number">
            <input type="submit" value="Search">
        </form>
        <% 
BookLogCrud book = new BookLogCrud();
ArrayList<BookLog> list = book.read();
...
&branch= <%=l.getBranch() %>&title=<%= l.getTitle() %>
&taken=<%= l.getTaken()%>&<%= l.getReceive() %>"> <i class="fas fa-edit" title="Edit"></i></a>
           <a href="logdelete.jsp?name=<%=l.getName() %>&rollno=<%=l.getRollno() %>
&branch= <%=l.getBranch() %>&title=<%= l.getTitle() %>
&taken=<%= l.getTaken()%>&<%= l.getReceive() %>">         
